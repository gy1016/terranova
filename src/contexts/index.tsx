...
const AppContextProvider = (props: PropsWithChildren) => {
  ...
  const [lang, setLang] = useState<'cn' | 'en'>(localStorageLang || (navigator.language.includes('en') ? 'en' : 'cn'));
  const [version, setVersion] = useState('latest');
  const [theme, setTheme] = useState(localStorageTheme);

  return (
    <AppContext.Provider value={{ lang, setLang, version, setVersion, theme, setTheme }}>
      {props.children}
    </AppContext.Provider>
  );
};
